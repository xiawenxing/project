...
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4])
{
	...
	*/
	...
	for (i = 0; i<4; i++)
	{
		...
		for (j = 0; j<4; j++)
		{
			ans[i] ^= aes_8bit_mul_mod_0x11B(a[i][j],y[j]);//a[i][j]* y[j]
		}
	}
...
void MixColumnInverse(unsigned char *p, unsigned char a[4], int do_mul)
{
	...
	for (j = 0; j<4; j++)
	{
		memcpy(b, &(p[4*j]), 4); /* 把第j行取出 */
		if (do_mul) /* 在加密最后一轮以及解密第一轮的MixColumn步骤中不需要做乘法; */
			aes_polynomial_mul(a, b, b); /* 其余轮都要做乘法: b = a*b mod (X^4+1); */

		put_column(b, t, (4 - j) % 4, j); /* 保存到t所指矩阵第j列;
										  第j列首元素的行号为j:
										  column 0: 0 1 2 3
										  column 3: 1 2 3 0
										  column 2: 2 3 0 1
...
{
	int i;
	unsigned char a[4] = { 0x0B, 0x0D, 0x09, 0x0E }; /* 定义多项式3*X^3+X^2+X+2的逆 */
	unsigned char matrix[4][4];
	memcpy(matrix, bufin, 4 * 4); /* 复制明文16字节到matrix */

	for (i = key_rounds; i >= 1; i--)
	{  /* 第1至key_rounds轮, 以下步骤: AddRoundKey,  MixColumn,ShiftRow, ByteSub */
		AddRoundKey((unsigned char *)matrix, key + i * (4 * 4));
		if (i != key_rounds)
