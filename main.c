...
static long32 f(ulong32 r, unsigned char subkey[8])
{
    ...
    // (2) 32位扩展成48位并保存到数组plaintext中(左边2位恒为0, 右边6位用来保存数据)
    memset(plaintext, 0, 8);
    for (i=0; i<8; i++)
    {
        for (j=0; j<6; j++)
        {
            b = plaintext_32bit_expanded_to_48bit_table[i*6+j]-1;
            by = b/8;
            bi = b%8;
            plaintext[i] |= (s[by] & bytebit[bi])? (bytebit[j]>>2) : 0;
        }
    }
    ...
    for (i=0; i<8; i++)
    {
        ...
        by = i/2;
        bi = i%2;
        m[by] |= bi ? sbox[i][rowcol] : sbox[i][rowcol]<<4;
    }
    
    // (5) 根据sbox_perm_table把m中的32位数打乱保存到数组t中
    memset(t, 0, 4);
    for (i=0; i<4; i++)
    {
        for (j=0; j<8; j++)
        {
            b = sbox_perm_table[4*i+j]-1;
            by = b/8;
            bi = b%8;
            t[i] |= (m[by] & bytebit[bi]) ? bytebit[j] : 0;
        }
    }
    
    // (6) 复制t到rval(注意大小端), 并返回rval。
    ptr = (unsigned char *)(&rval);
    for (i=0; i<4; i++)
    {
        *ptr++ = t[3-i];
    }
...
main()
{
    ulong32 c = 0x80800000;
    char subkey[8] = {1,1,1,1,1,1,1,1};
    char *b = &c;
    int i;
    des_init(0);
    c = f(c, subkey);
